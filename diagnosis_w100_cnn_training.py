...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(16, 32, 2, 1, 2), # -> (16, 25, 6)
            nn.ReLU(),
            nn.MaxPool2d(2),
        ) # -> (32, 13, 3)
        self.conv3 = nn.Sequential(
            nn.Conv2d(32, 32, 2, 1, 2), # -> (16, 25, 6)
            nn.ReLU(),
            nn.MaxPool2d(2),
        ) # -> (32, 13, 3)
...
data_output = {'cnn':{'F1':[], 'precision':[], 'recall':[], 'accuracy':[], 'auc':[], 'fpr':[], 'tpr':[], 'test_loss':[], 'train_loss':[], 'confusion_mat':[]}}
...
for num_of_training in range(KFOLD):
    ...
    for epoch in range(EPOCH):
        ...
        cnn_recall.append(sklearn.metrics.recall_score(cnn_final_test, cnn_final_prediction, average='macro').item())
    ...
    data_output['cnn']['confusion_mat'].append(current_cm)
...
for i in range(KFOLD):
    ...
    plt.savefig('../fig/w100_detection/'+'Loss_Kfold_CNN_w100_diagnosis_'+str(i+1)+'.png',dpi=500)
    ...
    plt.title('ROC curve')
    plt.savefig('../fig/w100_detection/'+'ROC_Kfold_CNN_w100_diagnosis_'+str(i+1)+'.png',dpi=500)
    
    
pickle_out = open('new_CNN_w100_10fold_diagnosis.pickle', 'wb')
