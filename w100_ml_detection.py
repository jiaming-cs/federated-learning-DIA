...
#data_path_FT = '../new_clean_data/w200_final_dataset/fault_diagnosis/feature/'

#all_data_FT = loader.waveformDiagnosisFeature(data_path_FT)

# detection deep learning dataset path
data_path_FT = '../new_clean_data/w100_final_dataset/fault_detection/feature/'
...
#     # train dtree
    dtree_model = DecisionTreeClassifier(max_depth = 12).fit(ml_X_train, ml_Y_train)
    dtree_pred = dtree_model.predict(ml_X_test)
    
    # train SVM 
    #svm_model = SVC(kernel = 'rbf', C = 1, max_iter = 100).fit(ml_X_train, ml_Y_train)
    svm_model = SVC(kernel = 'rbf', max_iter = 10).fit(ml_X_train, ml_Y_train)
    #svm_model = SVR(C=1.0, epsilon=0.2, max_iter=1000).fit(ml_X_train, ml_Y_train)
...
#     # metrics for machine learning methods
    dtree_f1 = sklearn.metrics.f1_score(ml_Y_test, dtree_pred, average='binary')
    dtree_precision = sklearn.metrics.precision_score(ml_Y_test, dtree_pred, average='binary')
    ...
    dtree_recall = sklearn.metrics.recall_score(ml_Y_test, dtree_pred, average='binary')
    dtree_acc = sklearn.metrics.accuracy_score(ml_Y_test, dtree_pred)
    
    svm_f1 = sklearn.metrics.f1_score(ml_Y_test, svm_pred, average='binary')
    svm_precision = sklearn.metrics.precision_score(ml_Y_test, svm_pred, average='binary')
    svm_recall = sklearn.metrics.recall_score(ml_Y_test, svm_pred, average='binary')
    #svm_recall = ((svm_f1*svm_precision) / (2*svm_precision - svm_f1))
    svm_acc = sklearn.metrics.accuracy_score(ml_Y_test, svm_pred)
    
    knn_f1 = sklearn.metrics.f1_score(ml_Y_test, knn_pred, average='binary')
    knn_precision = sklearn.metrics.precision_score(ml_Y_test, knn_pred, average='binary')
    knn_recall = sklearn.metrics.recall_score(ml_Y_test, knn_pred, average='binary')
...
for i in range(KFOLD):
    ...
    print('KNN: F1:{}, prec:{}, rec:{}, acc:{}'.format(data_output['knn']['F1'][i],
           data_output['knn']['precision'][i], data_output['knn']['recall'][i],
           data_output['knn']['accuracy'][i]))
    print('SVM: F1:{}, prec:{}, rec:{}, acc:{}'.format(data_output['svm']['F1'][i],
           data_output['svm']['precision'][i], data_output['svm']['recall'][i],
           data_output['svm']['accuracy'][i]))
    print('Dtree: F1:{}, prec:{}, rec:{}, acc:{}'.format(data_output['dtree']['F1'][i],
           data_output['dtree']['precision'][i], data_output['dtree']['recall'][i],
           data_output['dtree']['accuracy'][i]))

pickle_out = open('ML_w100_10fold_detection.pickle', 'wb')
