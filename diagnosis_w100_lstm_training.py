...
#data_path_DL = '../new_clean_data/w120_final_dataset/fault_detection/deep_learning/'
...
data_output = {'lstm':{'F1':[], 'precision':[], 'recall':[], 'accuracy':[], 'auc':[], 'fpr':[], 'tpr':[], 'test_loss':[], 'train_loss':[], 'confusion_mat':[]}}
...
for num_of_training in range(KFOLD):
    ...
    for epoch in range(EPOCH):
        ...
        current_f1 = sklearn.metrics.f1_score(lstm_final_test, lstm_final_prediction, average='weighted').item()
        current_precision = sklearn.metrics.precision_score(lstm_final_test, lstm_final_prediction, average='weighted', zero_division=1).item()
        lstm_f1_score.append(current_f1)
        lstm_precision.append(current_precision)
        lstm_recall.append((current_f1*current_precision) / (2*current_precision - current_f1))
    ...
    data_output['lstm']['confusion_mat'].append(current_cm)
...
for i in range(KFOLD):
    ...
    plt.savefig('../fig/w100_detection/'+'Loss_Kfold_LSTM_w100_diagnosis_'+str(i+1)+'.png',dpi=500)
    ...
    plt.title('ROC curve')
    plt.savefig('../fig/w100_detection/'+'ROC__Kfold_LSTM_w100_diagnosis_'+str(i+1)+'.png',dpi=500)
    
    
pickle_out = open('LSTM_w100_10fold_diagnosis.pickle', 'wb')
